...
module.exports = function resolveRunfiles(...pathSegments) {
  ...
  if (DEBUG) console.error('runfiles: try to resolve', pathSegments.join('/'));
  ...
  if (runfilesManifest) {
    ...
    if (maybe) {
      if (DEBUG) console.error('runfiles: resolved manifest directory', maybe);
      return maybe;
    }
  } else {
    ...
    if (maybe) {
      if (DEBUG) console.error('runfiles: resolved directory', maybe);
      return maybe;
    }
  }

  if (DEBUG) console.error('runfiles: resolved to default path', defaultPath);
  return defaultPath;
}