...
function symlink(target, path) {
  if (fs.existsSync(path)) {
    ...
    // This may occur when not running in a sandbox as the link may already exist from previous
    // run of linker. Could add some logic that asserts it here, but we want to avoid an extra
    // filesystem access so we should only do it under some kind of strict mode.
    log_verbose(`not symlinking ( ${path} -> ${target} ) which already exists`);
    return;
  }
