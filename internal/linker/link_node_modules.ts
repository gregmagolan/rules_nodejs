...
async function resolveExternalWorkspacePath(
  ...
  if (!execroot) {
    ...
    // Resolve as if we are in runfiles in a sandbox. This will be the case for nodejs_image.
...
export async function main(args: string[], runfiles: Runfiles) {
      ...
      if (await exists(workspaceNodeModules)) {
        ...
        if (!isExecroot) {
          ...
          if (await exists(runfilesPackagePath)) {
            await symlink(workspaceNodeModules, `${runfilesPackagePath}/node_modules`);
            resolvedPackagePath = runfilesPackagePath;
          }
        }
        if (await exists(packagePath)) {
          await symlink(workspaceNodeModules, `${packagePath}/node_modules`);
        ...
        if (resolvedPackagePath && await exists(packagePathBin)) {
          ...
          await symlink(`${resolvedPackagePath}/node_modules`, `${packagePathBin}/node_modules`);
        }
      }
    } else {
      // root node_modules
      if (await exists(workspaceNodeModules)) {
        await symlink(workspaceNodeModules, `node_modules`);
      } else {
  ...
  if (!rootNpmWorkspace) {
    // Special case if there is no root npm workspace, create a root node_modules folder for 1st
    // party deps
    log_verbose('no root npm workspace; creating node_modules directory in ', process.cwd());
    await mkdirp('node_modules');
  }

  // Change directory to the root node_modules directory for creation of 1st party deps symlinks
