...
"$SCRIPT_DIR/{node}" --preserve-symlinks --preserve-symlinks-main "$SCRIPT_DIR/{script}" "$SCRIPT_DIR" "$@"
""".format(
...
CALL "%SCRIPT_DIR%\\{node}" --preserve-symlinks --preserve-symlinks-main %*
...
#Generated by node_repositories.bzl
{get_script_dir}
"$SCRIPT_DIR/{node}" --preserve-symlinks --preserve-symlinks-main "$SCRIPT_DIR/{script}" --scripts-prepend-node-path=false "$@"
""".format(
                get_script_dir = GET_SCRIPT_DIR,
...
(cd "{root}"; "$SCRIPT_DIR/{node}" --preserve-symlinks --preserve-symlinks-main "$SCRIPT_DIR/{script}" --scripts-prepend-node-path=false "$@")
""".format(
...
echo Running npm %* in {root}
cd "{root}"
call "%SCRIPT_DIR%\\{node}" --preserve-symlinks --preserve-symlinks-main "%SCRIPT_DIR%\\{script}" --scripts-prepend-node-path=false %*
...
"{node}" --preserve-symlinks --preserve-symlinks-main "{script}" TMPL_args "$@"
""".format(
...
#Generated by node_repositories.bzl
{get_script_dir}
"$SCRIPT_DIR/{node}" --preserve-symlinks --preserve-symlinks-main "$SCRIPT_DIR/{script}" "$@"
""".format(
                get_script_dir = GET_SCRIPT_DIR,
                node = paths.relativize(node_entry, "bin"),
...
"$SCRIPT_DIR/{node}" --preserve-symlinks --preserve-symlinks-main "$SCRIPT_DIR/{script}" --cwd "{root}" "$@"
""".format(
                root = repository_ctx.path(package_json).dirname,
                node = paths.relativize(node_entry, "bin"),
                script = yarn_script_relative,
...
""" + "".join([
            """
echo Running yarn --cwd "{root}" %*
CALL "%SCRIPT_DIR%\\{node}" --preserve-symlinks --preserve-symlinks-main "%SCRIPT_DIR%\\{script}" --cwd "{root}" %*
if %errorlevel% neq 0 exit /b %errorlevel%
""".format(
                root = repository_ctx.path(package_json).dirname,
                node = paths.relativize(node_entry, "bin"),
